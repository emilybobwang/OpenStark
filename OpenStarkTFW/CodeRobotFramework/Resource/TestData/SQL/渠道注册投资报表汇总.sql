SELECT 
    ...
    CONCAT(CAST((SUM(tt.investorNum) / SUM(tt.registerNum) * 100) AS DECIMAL(10, 2)), '%') conversionsRates,
    '总计' createTime,
    SUM(tt.currentAmount) currentAmount,
    SUM(tt.regularAmount) regularAmount,
    SUM(tt.financierAmount) financierAmount,
    SUM(tt.investAmount) investAmount, 
    SUM(tt.investorNum) investorNum,
    SUM(tt.registerNum) registerNum
FROM 
(SELECT                                                                                                                                                                                                                        
...
GROUP BY ca.`accountingTime`,a.`channelId` ORDER BY ca.`accountingTime` DESC, a.`channelname` ASC LIMIT {rows}) AS tt;
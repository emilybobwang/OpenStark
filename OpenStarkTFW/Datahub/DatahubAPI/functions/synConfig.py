    ...
    def updateContent(self,sourceIp,targetIp,content):
        ...
        try:
            if content != '':
                ...
                ### -------------- app-6004-msg --------------
                # 替换6004上面发送短信和邮箱的配置
                content = content.replace("sms.default.channel=.*", "sms.default.channel=xuanwu400")
                ...
                # 修改为短信挡板
                content = content.replace("sms.default.channel=.*", "sms.default.channel=xuanwu400")
                content = content.replace("sms.channel.backup1=.*", "sms.channel.backup1=xuanwu400")
                content = content.replace("sms.normal.channel=.*", "sms.normal.channel=xuanwu400")
                content = content.replace("sms.channel.backup2=.*", "sms.channel.backup2=xuanwu400")

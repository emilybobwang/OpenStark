...
from functions.scheduler import jobs_status, run_jobs, close_network, sync_db_tables
...
def main():
    ...
    if options.monitor.lower() == 'on':
        ioloop.PeriodicCallback(jobs_status, cycle_time * 1000).start()
        ioloop.PeriodicCallback(run_jobs, cycle_time * 1000).start()
        ioloop.PeriodicCallback(close_network, cycle_time * 1000).start()
        ioloop.PeriodicCallback(sync_db_tables, cycle_time * 1000).start()

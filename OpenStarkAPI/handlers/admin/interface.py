...
class InterfaceHandler(BaseHandler):
    ...
    def get(self, op='overview', pid=0, page=1, limit=10):
        ...
        project_list = yield self.project.get_projects_by_user(user_id=self.current_user.email, status=1)
        lists = []
        total_page = 1
        if op == 'overview':
            jobs, total = yield self.setting.get_settings_list(
                s_type='job', user_id=self.current_user.email, pj_status=1, limit=limit, page=page)
            if pid != 0:
                jobs, total = yield self.setting.get_settings_list(
                    pid=pid, s_type='job', user_id=self.current_user.email, pj_status=1, limit=limit, page=page)
            ...
            if do == 'job':
                ...
                report, total = yield self.setting.get_settings_list(
                    s_type='report', user_id=self.current_user.email, status=1,
                    pj_status=1, limit=limit, page=page)
                if pid != 0:
                    report, total = yield self.setting.get_settings_list(
                        pid=pid, s_type='report', user_id=self.current_user.email, status=1,
                        pj_status=1, limit=limit, page=page)
                if self.current_user.role != 2:
                    project_list, total = yield self.project.get_projects_list(limit=None, status=1)
                    report, total = yield self.setting.get_settings_list(
            ...
            if do == 'list':
                ...
                if jobs and jobs.type == 'job':
                    ...
                    lists, total = yield self.setting.get_settings_list(pid=pid, s_type='suite',
                                                                        user_id=self.current_user.email, pj_status=1,
                                                                        limit=None)
                    if self.current_user.role != 2:
                ...
                lists, total = yield self.setting.get_settings_list(s_type='suite', user_id=self.current_user.email,
                                                                    pj_status=1, limit=limit, page=page)
                if pid != 0:
                    lists, total = yield self.setting.get_settings_list(pid=pid, s_type='suite',
                                                                        user_id=self.current_user.email, pj_status=1,
        ...
        if op == 'overview':
            ...
            if do in ['gethost', 'geturl']:
                if pid == 0:
                    ...
                    yield self.return_json(msg)
                    return
                msg = yield self.__get_url_by_project(pid=pid, do=do, user_id=self.current_user.email)
                if self.current_user.role != 2:
    ...
    def __get_url_by_project(self, pid, do='gethost', user=None):
        if do == 'gethost':
            hosts = []
            host_list, total = yield self.setting.get_settings_list(pid=pid, s_type='url', user_id=user, limit=None)
            ...
            ips = []
            ips_list, total = yield self.setting.get_settings_list(pid=pid, s_type='host', user_id=user,

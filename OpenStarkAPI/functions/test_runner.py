...
class TestRunner(object):
    ...
    def __run_all_test(self, job_name, test_suites):
        ...
        for test in test_suites:
            cases = yield self.setting.get_settings_by_id(test['cases'])
    ...
    @gen.coroutine
    def run_job(self):
        job = yield self.setting.get_settings_by_id(sid=self.jid)
        if job:
            start_time = time.time()
            ...
            test_suites = []
            suites = yield self.setting.get_settings_by_id(job_value['testsuite'])
            for suite in suites:

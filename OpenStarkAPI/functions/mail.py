...
class Mail(object):
    ...
    def _smtp_client(self):
        try:
            if self.mail_type == 'normal':
                ...
                return smtp, ''
            elif self.mail_type == 'exchange':
                credentials = Credentials(self.smtp_user, self.smtp_password)
                account = Account(self.mail_from, credentials=credentials, autodiscover=True)
                return account, ''
            else:
                return None, '邮箱类型错误'
    ...
    @gen.coroutine
    def send_mail(self, subject='[系统邮件]', message='', to=list(), cc=list()):
        ...
        if smtp_client is not None:
            if self.mail_type == 'normal':
                ...
                msg['To'] = '; '.join(to)
                msg['Cc'] = '; '.join(cc)
                msg['From'] = formataddr(parseaddr('系统邮件<{}>'.format(self.mail_from)))
                try:
                ...
                try:
                    to_mail = list()
                    for m in to:
                        to_mail.append(Mailbox(email_address=m))
                    mail = Message(account=smtp_client, subject=subject, body=HTMLBody(message),
                                   to_recipients=to_mail, cc_recipients=cc, folder=smtp_client.sent)
                    mail.send_and_save()
                    return True, ''
                except Exception as e:
                    log.warn(e)
    ...
    def send_html_report(self, rid):
        if rid:
            ...
            setting = yield settings.get_settings_by_id(rid)
            ...
            for i in range(len(lists)):
                ...
                if lists[i].result:
                    ...
                    param['result'] = '<span class ="label label-danger">未通过</span>'
                rows = """
                   <tr>
                       <td>{id}</td>
                       <td>{suite_name}</td>

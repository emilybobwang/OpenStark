...
class DESCrypt(object):
    ...
    # DES加密
    def des_encode(self, messages, encode='utf8'):
        if self.mode == DES.MODE_ECB:
            obj = DES.new(self.key, self.mode)
        else:
            obj = DES.new(self.key, self.mode, self.iv)
        return obj.encrypt(self.pad(messages).encode(encode)).hex()

    # DES解密
    def des_decode(self, des_str, decode='utf8'):
        if self.mode == DES.MODE_ECB:
            obj = DES.new(self.key, self.mode)
        else:
            obj = DES.new(self.key, self.mode, self.iv)
        return self.unpad(obj.decrypt(bytes.fromhex(des_str)).decode(decode, errors='ignore'))

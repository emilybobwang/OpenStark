...
class Keyboard(object):
    ...
    def __get_winio(self):
        if self.__winio is None:
            self.__winio = rabird.winio.WinIO()
    ...
    def key_input(self, chars='', press_time=0.1):
        for c in chars:
            try:
                self.key_press(self.VK_CODE[c], press_time)
            except:
                self.key_press(0x3A, press_time)
                time.sleep(0.5)
                self.key_press(self.VK_CODE[c.lower()], press_time)
                self.key_press(0x3A, press_time)

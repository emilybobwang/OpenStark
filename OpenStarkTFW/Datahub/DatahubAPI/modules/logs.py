...
class LogsModule(object):
    ...
    # 添加配置项
    @gen.coroutine
    def add_log(self, s_type, name, value, pid=None, project=None,
        ...
        if create_time is not None:
            ...
            time_param = ''
            time_value = ''
        sql = """INSERT INTO t_logs (`projectId`, `type`, `name`, `value`, `status`{})
        VALUE (({}), %(s_type)s, %(name)s, %(value)s, %(status)s{})""".format(time_param, pid, time_value)
        if pid:
    ...
    # 删除配置项
    @gen.coroutine
    def delete_logs(self, sid):
        setting = yield self.get_logs_by_id(sid=sid)
        if isinstance(sid, list):
            sid = ','.join([str(s) for s in sid])
        if setting and sid:
            sql = 'DELETE FROM t_logs WHERE id in ({})'.format(sid)
            tx = yield pool.begin()
            try:
    ...
    # 按类型批量删除配置项
    @gen.coroutine
    def delete_logs_by_type(self, s_type, name=None, status=None):
        setting = yield self.get_logs_list(s_type=s_type, status=status, name=name)
        if setting:
            where = []
            ...
            if status is not None:
                ...
                param['name'] = name
            sql = 'DELETE FROM t_logs WHERE type=%(s_type)s'
            if where:
    ...
    # 编辑配置项
    @gen.coroutine
    def edit_log(self, sid, status=None, name=None, pid=None, project=None, value=None, create_time=None):
        if project is not None:
            pid = "SELECT p.id FROM t_projects p WHERE p.`name`='{}' and p.type='project'".format(project)
        update = []
        param = dict(sid=sid)
        ...
        if update and setting:
            sql = 'UPDATE t_logs s SET {} WHERE id=%(sid)s'.format(', '.join(update))
            tx = yield pool.begin()
            try:
                yield tx.execute(sql, param)
            except pymysql.Error as e:
    ...
    # 通过条件获取配置项
    @gen.coroutine
    def get_log(self, pid=None, name=None, s_type=None, project=None,
                    status=None, pj_status=None):
        sql = """
            SELECT s.id, s.projectId project_id, s.type, s.name, s.value, s.status, p.name project_name, s.createTime
            FROM t_logs s JOIN t_projects p ON s.projectId=p.id
            """
        where = []
        param = dict()
        if pid is not None:
    ...
    # 通过条件获取配置项列表
    @gen.coroutine
    def get_logs_list(self, pid=None, name=None, s_type=None, order_by=list(), project=None,
                          status=None, pj_status=None, page=1, limit=None, search=None, create_time=None):
        sql = """
            SELECT s.id, s.projectId project_id, s.type, s.name, s.value, s.status, p.name project_name, s.createTime
            FROM t_logs s JOIN t_projects p ON s.projectId=p.id
            """
        sql_count = 'SELECT COUNT(*) count FROM t_logs s JOIN t_projects p ON s.projectId=p.id'
        where = []
        param = dict()
        if pid is not None:
            if isinstance(pid, list):
        ...
        if name is not None:
            ...
            param['name'] = name
        if create_time is not None:
            where.append("s.createTime BETWEEN %(startTime)s AND %(endTime)s")
            param['startTime'] = '{} 00:00:00'.format(create_time)
            param['endTime'] = '{} 23:59:59'.format(create_time)
        if s_type is not None:
    ...
    # 通过id获取配置项
    @gen.coroutine
    def get_logs_by_id(self, sid, order_by=list()):
        if isinstance(sid, list):
            sid = ','.join([str(s) for s in sid])
        if sid:
            sql = """
                SELECT s.id, s.projectId project_id, s.type, s.name, s.value, s.status, p.name project_name, s.createTime
                FROM t_logs s JOIN t_projects p ON s.projectId=p.id WHERE s.id IN ({})
                """.format(sid)
            if order_by:
                sql += ' ORDER BY {}'.format(','.join(order_by))
            try:

...
class OptionsFunction(object):
    ...
    def get_crypt_info(self, pid, do='encrypt'):
        setting, total = yield self.setting.get_logs_list(pid=pid, s_type='crypt', limit=None)
    ...
    def get_custom_param(self, pid, correlation={}):
        setting, total = yield self.setting.get_logs_list(pid=pid, s_type='param', limit=None)
        setting = setting[0] if len(setting) > 0 else None
        if setting:
            params_str = []
            params_data = []
        ...
        if len(sql_params) == 5:
            for line in sql_params:
                ...
                if line[0].strip() in ['mysql']:
                    ...
                    ips, total = yield self.setting.get_logs_list(
    ...
    def get_check_key(self, pid, url):
        setting, total = yield self.setting.get_logs_list(pid=pid, s_type='url', limit=None)
        check_key = ''
        if setting:
            for row in setting:

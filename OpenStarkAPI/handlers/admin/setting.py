        ...
        if op == 'project':
            ...
            if lists:
                ...
                for row in lists:
                    ...
                    project['user'] = self.__get_setting_project_user_email(row.userId)
            ...
            if pid != 0:
                lists, total = yield self.setting.get_settings_list(
                    pid=pid, s_type='host', user_id=self.current_user.email, pj_status=1, limit=limit, page=page)
            ...
            lists, total = yield self.setting.get_settings_list(
                s_type='url', page=page, limit=limit, user_id=self.current_user.email, pj_status=1)
            if pid != 0:
                lists, total = yield self.setting.get_settings_list(
                    pid=pid, s_type='url', user_id=self.current_user.email, pj_status=1, limit=limit, page=page)
            if self.current_user.role != 2:
                project_list, total = yield self.project.get_projects_list(limit=None, status=1)
            ...
            lists, total = yield self.setting.get_settings_list(
                s_type='crypt', page=page, limit=limit, user_id=self.current_user.email, pj_status=1)
            if self.current_user.role != 2:
                project_list, total = yield self.project.get_projects_list(limit=None, status=1)
    ...
    def __setting_host(self, do=''):
        if do == 'add':
            ...
            if not self.common_func.check_string(ip, 'ip') or not self.common_func.check_string(host, 'host'):
                ...
                return
            if pid == 0:
                project = yield self.project.get_projects_by_user(user_id=self.current_user.email, status=1)
                if self.current_user.role != 2:
